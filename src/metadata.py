...
# Instantiate Jinja2
j2 = Environment(loader=FileSystemLoader(os.path.join(os.path.dirname(__file__), 'templates')))
...
def get_ms_device_metadata_filename(vid, pid):
    ...
    payload = j2.get_template('ms_request_body.tpl').render( 
        vid=format(vid, 'x').zfill(4), 
        pid=format(pid, 'x').zfill(4)
    ).encode('UTF-16LE')
    headers = {
        ...
        'SOAPAction': 
            'http://schemas.microsoft.com/windowsmetadata/services/2007/09/18/dms/DeviceMetadataService/GetDeviceMetadata', 
